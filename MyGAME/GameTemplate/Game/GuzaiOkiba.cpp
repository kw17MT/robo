	...
	level.Init("Assets/level/level_new.tkl", [&](ObjectData& objectData) {
		if (wcsncmp(objectData.name, StockRight, 10) == 0) {
			...
			case 1: {
				...
				stock[m_kitchenNo - 1] = NewGO<Kitchen>(0);
				stock[m_kitchenNo - 1]->SetKitchenPos(m_kitchenPos[m_kitchenNo - 1]);
				return true;
			}break;
			case 2: {
				m_kitchenPos[m_kitchenNo - 1] = objectData.Pos;
		...
		if (wcsncmp(objectData.name, StockLeft, 9) == 0) {
			...
			case 1: {
				...
				stock[m_kitchenNo + 3] = NewGO<Kitchen>(0);
				stock[m_kitchenNo + 3]->SetKitchenPos(m_kitchenPos[m_kitchenNo + 3]);
				return true;
			}break;
			case 2: {
				m_kitchenPos[m_kitchenNo + 3] = objectData.Pos;
